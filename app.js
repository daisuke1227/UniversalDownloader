...
const downloadSingleItem = async (job, url, options, outTemplate) => {
  ...
  const subtitleArgs = includeSubtitles
    ? ['--write-auto-subs', '--write-subs', '--embed-subs', '--sub-langs', 'en.*', '--convert-subs', 'srt']
    : [];

  const thumbnailArgs = format !== 'wav'
...
const handlePlaylistDownload = async (job, url, entries, playlistTitle, options) => {
  ...
  if (videoCount > 50) {
    sleepArgs = ['--sleep-interval', '5', '--max-sleep-interval', '10'];
  }
...
const handleSingleDownload = async (job, metadata, options) => {
  ...
  job.filePath = await downloadSingleItem(job, url, options, outTemplate);
};
    ...
    for (const entry of entries) {
      if (entry._type === 'playlist' && entry.entries) {
        ...
        if (!processedUrls.has(entry.url)) {
          playlistsToProcess.push(entry.url);
        }
      } else if (entry.url) {
